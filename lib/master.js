...
class Master extends Event{
    ...
    tryConnect ( id ,ips , port, callback ){
        ...
        client.on('connect',function(){
            ...
            this._invoke("@@WORKERINFOS",{},function( status, err , result ){
                client.routes = result.routes;
                client.opts = { name: result.name, detail : result.detail };
            });
        });
    ...
    invoke ( method, request , callback ){
        ...
        let self = this;
        if(!client){
            return callback( null, this.toResult(404,new Error(`Url : [${serviceId||method}] not found!`)));
        }
        client.invoke( _method, request, function( status, err, msg){
            callback(null,self.toResult( status, err, msg ));
        });
        return this;
    }
    ...
    readme ( serviceId, callback ){
        ...
        if(worker){
            return callback( null, this.toResult(200,null,{ routes: worker.routes, name: worker.opts.name, detail: worker.opts.detail }));
        }
    ...
    ask ( method , callback ){
        ...
        return callback( null, this.toResult( 200, null, client.toJSON() ));
    }
    
    getServiceID ( method ){
        var result = {};
        var item = Object.keys(this.workers).forEachExt(function(key,index){
            if(0 === method.indexOf(key)){
                return true;
            }
        });
        result.id = item;
        result.method = method.replace(item,'');
        return result;
