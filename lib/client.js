...
class Client extends EventEmitter {
    ...
    toResult(code, err, result) {
        let msg = JSON.stringify({
            status: code,
            err: err ? {message: err.message, stack: err.stack, errors: err.errors} : undefined,
            result: result
        });
        return msg;
    }
    ...
    invoke(method, request, callback) {
        let self = this;
        this.ask(method, function (err, myclient) {
            if (err) {
                return callback(self.toResult(404,err));
            } else if(myclient){
                myclient._invoke( method.replace(myclient.serviceId,''), request, callback );
            } else{
                return callback()
            }
        });
    ...
    _invoke( method, request, callback) {
        if ('object' == typeof(request)) {
            request = JSON.stringify(request);
        }
        this.client.invoke(method, request, function (err, result) {
    ...
    ask(method, callback) {
        let self = this;
        var tmpclient = Object.keys(self.maps).forEachExt(function(item){
            if(0 == method.indexOf(item)){
                debug('from cache');
                return true;
            }
        });
        if(tmpclient){
            return callback(null,self.maps[tmpclient]);
        }
        if(null==this.client){
            var error = new Error('the client has been close');
            error.status = 404;
        ...
        this.client.ask(method, function (err, result) {
            ...
            if (200 == data.status) {
                ...
                client.on('connect', function () {
                    ...
                    debug('connect to ', this.serviceId);
