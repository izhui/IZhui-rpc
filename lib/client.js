...
class Client extends EventEmitter{
    ...
    invoke ( method, request, callback ){
        ...
        if(this.client){
            this.client.invoke( method, request, function( err , result ){
                ...
                callback( null, self.toResult(result.status, result.err, result.result));
            });
        }else{
            callback(null,this.toResult(409,new Error(`the master not connect yet!`)));
        }
        return this;
    }
    ...
    workerList ( callback ){
        ...
        this.client.workerList( function( err, result ){
            let data = JSON.parse(result);
            return callback(null,self.toResult( data.status, data.err, data.result ));
        });
    }
